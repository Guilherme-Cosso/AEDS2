...
class Classe {
  ...
  public static void main(String[] args) {
    ...
    p = saveArk();
    int[] id = PubInNumbers();
    String[] nomesId = LerId(p, id);
    String[] nomes = PubInVector();
    // System.out.println(Arrays.toString(nomes));
    // System.out.println(Arrays.toString(id));
    // System.out.println(Arrays.toString(nomesId));
    Printar(nomesId, nomes);
  ...
  /*
   * @ method: Ler a 1 parte "Numeros" dos PubIn e salva em um vector
   * 
   * @ return: vetor de Ids
   */
  ...
  public static String tratarNomes(String nome){
    String str="";
    if (nome.contains("*")) {
  ...
  /*
   * @ method: Retira os * dos Nomes
   * 
   * @ return: vetor de Nomes
   */
  public static String[] LerId(Player[] p, int[] id) {
    String[] nomes = new String[tamId];
    for (int i = 0; i < tamId; i++) {
      nomes[i] = p[id[i]].getNome();
      nomes[i]= tratarNomes(nomes[i]);
    }
    return nomes;
  }

  /*
   * @ method: Ler a 2 parte "Nomes" do PubIn e salva em um vector
   * 
   * @ return: vetor de Nomes
   */
  public static String[] PubInVector() {
    String dados = "", str = MyIO.readLine();
    while (!str.contains("FIM")) {
      dados += str + ",";
      tamNomes++;
    ...
    for (int i = 0; i < tamNomes; i++) {
      vector[i]= tratarNomes(vector[i]);
    }
    return vector;
  }

  public static void atualizarIds(String[] vector, int j, int aux) {
    for (int l = j; l < tamId - aux; l++)
      vector[l] = vector[l + 1];
    for (int l = tamId - aux; l < tamId; l++)
      vector[l] = null;
  }


  public static void Printar(String[] dadoID, String[] nomes) {
    Boolean resp = false;
    String str = "";
    int aux = 0;
    ...
    for (int i = 0; i < tamNomes; i++) {
      resp = false;
      for (int j = 0; j < tamId - aux; j++) {
        str = dadoID[j];
        comp++;
        // MyIO.println(str+"=="+nomes[i]+" Id"+dados[j] );
        // MyIO.println(str.equals(nomes[i]));
        if (str.equals(nomes[i])) {
          // MyIO.println(str);
          aux++;
          atualizarIds(dadoID, j, aux);
          // MyIO.println("-____________-");
          resp = true;
        }
      }
      if (resp == true)

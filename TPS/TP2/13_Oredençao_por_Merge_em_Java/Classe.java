...
class Classe {
    ...
    public static void sort(int array[], int l, int r, Player[] p) 
    { 
        if (l < r) { 
            // Achar o meio
            int m = (l + r) / 2; 
    ...
    public static void merge(int[] id,int l, int m, int r ,Player[] p) {
        ...
        int k = l; 
        while (i < n1 && j < n2) { 
            if ( p[L[i]].getUniversidade().compareTo(p[R[j]].getUniversidade())<0 || ( p[L[i]].getUniversidade().compareTo(p[R[j]].getUniversidade())== 0 &&  p[L[i]].getNome().compareTo(p[R[j]].getNome())< 0) ){ 
                id[k] = L[i]; 
                i++; 
            } 
            else { 

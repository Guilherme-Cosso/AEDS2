...
void quicksort(Jogador p[], int *array, int esq, int dir);
...
void quicksort(Jogador p[], int *array, int esq, int dir)
{
  ...
  while (i <= j)
  {
    while (strcmp(p[array[i]].estadoNascimento, p[pivo].estadoNascimento) < 0 || (strcmp(p[array[i]].estadoNascimento, p[pivo].estadoNascimento) == 0 && strcmp(p[array[i]].nome, p[pivo].nome) < 0))
      i++;
    while (strcmp(p[array[j]].estadoNascimento, p[pivo].estadoNascimento) > 0 || (strcmp(p[array[j]].estadoNascimento, p[pivo].estadoNascimento) == 0 && strcmp(p[array[j]].nome, p[pivo].nome) > 0))
      j--;
    if (i <= j)
    {
  ...
  if (esq < j)
    quicksort(p, array, esq, j);
  if (i < dir)
    quicksort(p, array, i, dir);
}

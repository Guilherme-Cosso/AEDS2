...
class Classe {
    ...
    public static void quicksort(int[] id, int esq, int dir, Player[] p) {
        int i = esq, j = dir;
        String pivo = p[id[(esq + dir) / 2]].getEstadoNascimento(), pivos = p[id[(esq + dir) / 2]].getNome();
        while (i <= j) {
            while (p[id[i]].getEstadoNascimento().compareTo(pivo) < 0
                    || (p[id[i]].getEstadoNascimento().compareTo(pivo) == 0 && p[id[i]].getNome().compareTo(pivos) < 0))
                i++;
            while (p[id[j]].getEstadoNascimento().compareTo(pivo) > 0
                    || (p[id[j]].getEstadoNascimento().compareTo(pivo) == 0 && p[id[j]].getNome().compareTo(pivos) > 0))
                j--;
            if (i <= j) {
                swap(i, j, id);
                i++;
        ...
        if (esq < j)
            quicksort(id, esq, j, p);
        if (i < dir && i < 10)
            quicksort(id, i, dir, p);
    }
